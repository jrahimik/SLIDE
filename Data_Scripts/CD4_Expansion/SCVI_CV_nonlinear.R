...
Z <- read.table("/ix/djishnu/Javad/CDexpansion/SCVI/scvi_nonlinear_factors.csv", sep = " ")
...
for (r in 1:replicates) {
  ...
  indices <- sample(1:k, nrow(data_shuffled), replace = TRUE)
  predictions <- rep(0, nrow(data_shuffled))
...
saveRDS(cor_list,file="/ix/djishnu/Javad/CDexpansion/SCVI/scvi_cross_vae_validated.rds")
